...
async function loadSummary() {
    try {
        const res = await fetch('/api/summary', { headers: getHeaders() });
        const data = await res.json();
...
async function loadAnalytics() {
    try {
        const res = await fetch('/api/summary', { headers: getHeaders() });
        const data = await res.json();
        const txRes = await fetch('/api/transactions', { headers: getHeaders() });
...
async function deleteDeal(id) {
    ...
    await fetch(`/api/deals/${id}`, { method: 'DELETE', headers: getHeaders() });
...
async function deleteBudget(id) {
    ...
    await fetch(`/api/budgets/${id}`, { method: 'DELETE', headers: getHeaders() });
...
async function deleteGoal(id) {
    ...
    await fetch(`/api/goals/${id}`, { method: 'DELETE', headers: getHeaders() });
...
async function contributeToGoal(id, current) {
    ...
    const goal = (await (await fetch('/api/goals', { headers: getHeaders() })).json()).goals.find(g => g.id === id);
    if (!goal) return;
...
async function seedDemoData() {
    try {
        const res = await fetch('/api/seed', { method: 'POST', headers: getHeaders() });
        if (res.ok) {
            triggerBouncyCelebration();
...
async function exportDataJSON() {
    const res = await fetch('/api/export/json', { headers: getHeaders() });
    const data = await res.json();
